...
before(async () => {
  const config = { ...loadConfig({}), reportRateLimit: 3, reportRateWindowMin: 10 };
  ...
  const app = createApp({ config, kma, shelters, reportStore: store, events });
...
test('static frontend, locales and Leaflet are served', async () => {
  assert.match(await (await fetch(`${base}/`)).text(), /id="map"/);
